...
def add_train_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    Declared once so `astro-mine-train` and the umbrella's `astro-mine train` (RFC-0011 §3, wired
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="astro-mine-learn", description=__doc__)
...
def main(argv: Sequence[str] | None = None) -> int:
    return run_from_args(_parser().parse_args(argv))
...
def deprecated_alias(argv: Sequence[str] | None = None) -> int:
    """The pre-RFC-0011 ``astro-mine-train`` name — kept for one deprecation cycle.

    Unlike the bare names this rename also swept up (``fleet``, ``worlds``, ``link``,
    ``prospect``), ``astro-mine-train`` was never a ``PATH`` land-grab — it already carried the
    prefix. What it got wrong was the noun: it was named after the *verb* it runs rather than the
    package that ships it, so the package-to-command mapping every other component follows broke
    here alone (``conventions.md §13``; RFC-0011 §5). ``astro-mine-learn`` restores it.

    The old name keeps working, prints one line naming its replacement, and is **removed at the
    first public-benchmark milestone** — before the platform is public, so no outside user ever
    learns the transitional name.

    The notice goes to **stderr**, never stdout: a training run's report JSON goes to stdout, and
    a warning mixed into it would corrupt exactly the pipelines most likely to still use the old
    name in a script.
    """
    print(
        "warning: `astro-mine-train` is deprecated and will be removed at the first "
        "public-benchmark "
        "milestone; use `astro-mine-learn` instead (RFC-0011 §5).",
        file=sys.stderr,
    )
    return main(argv)
