...
(conventions.md §7), and each real sibling registers there **from its own side** — so co-installing
``astro-mine-mind`` and ``astro-mine-guard[mind]`` wires Mind's mandatory shield stage
(RM-P1-MIND-05) to the *real* Rust TCB, with **no** ``mind → guard`` dependency and no edit to
either package's core. Base Guard imports nothing from Mind; base Mind ships its own reference
...
Importing this module without ``astro-mine-mind`` installed raises :class:`ImportError` — by
construction, since only Mind's own registry ever loads the entry point.
"""
