...
2. **Remote-capable producer CLIs.** Each producer's `publish` must accept a remote registry.
   This is the one-line `Registry(path)` → `open_registry(location)` change tracked as the
   companion PRs on `astro-mine-worlds` / `-fleet` / `-prospect` / `-link`. Until they merge,
   producers publish to a local registry only.
...
python astro-mine-link/scripts/build_anchor_contact_plan.py --registry "$REGISTRY" --key "$ORG_KEY" …
```
