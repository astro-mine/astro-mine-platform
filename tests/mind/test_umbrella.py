"""Mind's adapter for the umbrella CLI (`astro-mine mind`).
...
the same ``main`` `astro-mine-mind` runs. What is worth pinning is that the tail arrives
...
from astro_mine.mind import umbrella
...
def test_adapter_satisfies_the_structural_contract() -> None:
    for member in ("name", "help", "add_arguments", "run"):
        assert hasattr(umbrella.mind, member), f"missing {member!r}"
    assert callable(umbrella.mind.add_arguments)
    assert callable(umbrella.mind.run)
    assert umbrella.mind.name == "mind"
    assert umbrella.mind.help
...
def test_the_entry_point_resolves_and_matches_its_name() -> None:
    ...
    (ours,) = [
        ...
        if ep.value.startswith("astro_mine.mind.umbrella:")
    ]
    assert ours.name == "mind"
...
def test_the_tail_arrives_intact() -> None:
    ...
    reach Mind's own parser unmangled — including ones the umbrella has never heard of."""
    parser = argparse.ArgumentParser(prog="astro-mine mind")
    umbrella.mind.add_arguments(parser)
    args = parser.parse_args(["stacks", "--some-flag", "value", "positional"])
    assert args.tail == ["stacks", "--some-flag", "value", "positional"]
...
def test_the_exit_status_survives_the_handoff(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    parser = argparse.ArgumentParser(prog="astro-mine mind")
    umbrella.mind.add_arguments(parser)
    assert umbrella.mind.run(parser.parse_args(["stacks"])) == 3


def test_a_usage_error_becomes_a_status_not_an_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    """argparse inside Mind raises SystemExit; the umbrella's contract is a returned status, so
    ...
    parser = argparse.ArgumentParser(prog="astro-mine mind")
    umbrella.mind.add_arguments(parser)
    assert umbrella.mind.run(parser.parse_args(["stacks"])) == 2
...
def test_a_clean_systemexit_is_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    parser = argparse.ArgumentParser(prog="astro-mine mind")
    umbrella.mind.add_arguments(parser)
    assert umbrella.mind.run(parser.parse_args(["stacks"])) == 0
