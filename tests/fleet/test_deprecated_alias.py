"""The deprecated `fleet` alias (RFC-0011 §5).

`fleet` was a generic name on every user's PATH; the platform now names a component's command
...
from astro_mine.fleet.cli import deprecated_alias, main

OLD = "fleet"
NEW = "astro-mine-fleet"
...
def test_both_console_scripts_are_declared() -> None:
    ...
    assert scripts.get(NEW) == "astro_mine.fleet.cli:main"
    assert scripts.get(OLD) == "astro_mine.fleet.cli:deprecated_alias"
