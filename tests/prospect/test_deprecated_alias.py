"""The deprecated `prospect` alias (RFC-0011 §5).

`prospect` was a generic name on every user's PATH; the platform now names a component's command
...
from astro_mine.prospect.publish import deprecated_alias, main

OLD = "prospect"
NEW = "astro-mine-prospect"
...
def test_both_console_scripts_are_declared() -> None:
    ...
    assert scripts.get(NEW) == "astro_mine.prospect.publish:main"
    assert scripts.get(OLD) == "astro_mine.prospect.publish:deprecated_alias"
