"""The deprecated `link` alias (RFC-0011 §5).

`link` was a generic name on every user's PATH; the platform now names a component's command
...
from astro_mine.link.cli import deprecated_alias, main
...
def test_both_console_scripts_are_declared() -> None:
    ...
    assert scripts.get(NEW) == "astro_mine.link.cli:main"
    assert scripts.get(OLD) == "astro_mine.link.cli:deprecated_alias"
