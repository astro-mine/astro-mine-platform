"""Link's adapter for the umbrella CLI (`astro-mine link`).
...
the same ``main`` `link` runs. What is worth pinning is that the tail arrives
...
from astro_mine.link import umbrella
...
def test_adapter_satisfies_the_structural_contract() -> None:
    for member in ("name", "help", "add_arguments", "run"):
        assert hasattr(umbrella.link, member), f"missing {member!r}"
    assert callable(umbrella.link.add_arguments)
    assert callable(umbrella.link.run)
    assert umbrella.link.name == "link"
    assert umbrella.link.help
...
def test_the_entry_point_resolves_and_matches_its_name() -> None:
    ...
    (ours,) = [
        ...
        if ep.value.startswith("astro_mine.link.umbrella:")
    ]
    assert ours.name == "link"
...
def test_the_tail_arrives_intact() -> None:
    ...
    reach Link's own parser unmangled — including ones the umbrella has never heard of."""
    parser = argparse.ArgumentParser(prog="astro-mine link")
    umbrella.link.add_arguments(parser)
...
def test_the_exit_status_survives_the_handoff(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    parser = argparse.ArgumentParser(prog="astro-mine link")
    umbrella.link.add_arguments(parser)
    assert umbrella.link.run(parser.parse_args(["publish"])) == 3


def test_a_usage_error_becomes_a_status_not_an_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    """argparse inside Link raises SystemExit; the umbrella's contract is a returned status, so
    ...
    parser = argparse.ArgumentParser(prog="astro-mine link")
    umbrella.link.add_arguments(parser)
    assert umbrella.link.run(parser.parse_args(["publish"])) == 2
...
def test_a_clean_systemexit_is_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    parser = argparse.ArgumentParser(prog="astro-mine link")
    umbrella.link.add_arguments(parser)
    assert umbrella.link.run(parser.parse_args(["publish"])) == 0
